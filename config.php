...
DEFINE('DBUSR', 'root');
DEFINE('DBPASS', '');
DEFINE('DB', 'duty_roaster_info_for_nurses');
    ...
    public function bouncer()
    {
        if (isset($_SESSION['auth'])) {
            ...
            if ($user->num_rows > 0) {
                ...
                $user = $this->db->query("UPDATE users SET status='Nurse' WHERE email='$auth' AND type =1 ");
                $user = $this->db->query("UPDATE users SET status='Admin' WHERE email='$auth' AND type=9");
                $user = $this->db->query("UPDATE users SET status='Supervisor' WHERE email='$auth' AND type=5");
            } else {
...
$project = new project("Duty Roaster For Nurses");
